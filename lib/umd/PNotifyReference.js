(function (global, factory) {
	if (typeof define === "function" && define.amd) {
		define("PNotifyReference", ["exports", "./PNotify.html"], factory);
	...
	function oncreate() {
		...
		// This event is fired from the core to give us our actual options. Until
		// it is fired, our options will just be the defaults.
		...
		// We have to fire this event to tell the core that we are ready to
		// receive our options.
	...
	function setup(Component) {
		...
		_extends(_PNotify2.default.styling.fontawesome, {
			athing: "fa fa-refresh"
		});
	};

	function encapsulateStyles(node) {
		setAttribute(node, "svelte-4250733081", "");
	}

	function add_css() {
		var style = createElement("style");
		style.id = 'svelte-4250733081-style';
		style.textContent = ".ui-pnotify-reference-button[svelte-4250733081]{float:right}.ui-pnotify-reference-clearing[svelte-4250733081]{clear:right;line-height:0}";
	...
	function create_main_fragment(state, component) {
		...
		var if_block = state.put_thing && create_if_block(state, component);
	...
	function create_if_block(state, component) {
		...
		return {
			...
			h: function hydrate() {
				...
				encapsulateStyles(i);
				i.className = i_class_value = state._notice.get('_styles').athing;
			...
			p: function update(changed, state) {
				if (changed._notice && i_class_value !== (i_class_value = state._notice.get('_styles').athing)) {
	...
	function PNotifyReference(options) {
		...
		if (!document.getElementById("svelte-4250733081-style")) add_css();
